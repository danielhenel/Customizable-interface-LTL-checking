...
#             m.file = open(os.path.join(cwd,"test_input", check_file),"r")
...
class Four_eyes_principle(unittest.TestCase):
    def testfour_eyes_principle(self):
        ...
        df.rename(columns={"Incident ID":"case:concept:name", "IncidentActivity_Type" : "concept:name", "DateStamp" : "time:timestamp", "Assignment Group" : "org:resource"}, inplace = True)
        temp = four_eyes_principle(df,["Open","Closed"])
        
        self.assertIsInstance(temp, pd.DataFrame)
        # with self.assertRaises(ValueError):
        #     four_eyes_principle(df, ["Open", "Closed"])
...
class Simplify_expression(unittest.TestCase):
    def test1(self):
        self.assertEqual(str(simplifyExpression(sympify("(E & A & F ) | ( A & F )",_clash1))),str([[A],[F]]))

    def test2(self):
        self.assertEqual(str(simplifyExpression(sympify("(A & E) | (F & E | A) & (A | F)",_clash1))),str([[A,E],[A,F]]))
    
    def test3(self):
        self.assertEqual(str(simplifyExpression(sympify("((A | F) | E & (A | E) & (F | E)) & (A & E | F & (E | A))",_clash1))),str([[A,E],[A,F],[E,F]]))

    def test4(self):
        self.assertEqual(str(simplifyExpression(sympify("E",_clash1))),str([[E]]))

    def test5(self):
        self.assertEqual(str(simplifyExpression(sympify("A & F",_clash1))),str([[A],[F]]))
